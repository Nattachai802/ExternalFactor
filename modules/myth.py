...
- แต่ละรายการที่ตอบต้องไม่ซ้ำกัน ห้ามตอบรายการเดิมหลายครั้งเพื่อให้ครบจำนวน
...
# คำปิดท้าย/คำปฏิเสธที่ปนมาในรายการ 宜 — ไม่ใช่กิจกรรมที่ทำได้ ตัดก่อนให้ LLM เลือก
# (เจอจริง 18 ก.ย. 2026: 宜 มีแต่พิธีศพ + 馀事勿取 → LLM คว้าคำปิดท้ายมาตอบซ้ำ 3 ครั้ง)
NOT_ACTIVITY = {"กิจอื่นไม่ควรทำ", "ไม่เหมาะกับทุกกิจ", "ไม่มี"}
...
def pick_recommended(yi_words: list[str], asker=None) -> list[str]:
    ...
    วันที่ 宜 มีแต่คำปิดท้าย (ไม่มีกิจกรรมจริงเลย) → คืน [] ยัดคำปฏิเสธไปแสดงไม่ได้
    """
    # dict.fromkeys = กันซ้ำโดยรักษาลำดับเดิม
    words = list(dict.fromkeys(w for w in yi_words if w and w not in NOT_ACTIVITY))
    if not words:
        return []
    if len(words) <= 3:
        return words
    ...
    # กันโมเดลแต่งคำเอง/โดนข้อความในเว็บสั่ง — เก็บเฉพาะคำที่อยู่ในรายการจริง และไม่ซ้ำ
    picked = list(dict.fromkeys(p.strip() for p in answer.split("|") if p.strip() in words))

    # LLM มักตอบไม่ครบตอนรายการไม่เกี่ยวกับร้านค้าเลย (วันพิธีศพ 18 ก.ย. 2026 ตอบมาข้อเดียว)
    # สเปกบอกต้องส่ง 2-3 ข้อเสมอ — เติมจากรายการตามลำดับต้นทาง ไม่ปล่อยให้การ์ดโล่ง
    for w in words:
        if len(picked) >= 2:
            break
        if w not in picked:
...
def demo():
    ...
    assert pick_recommended(broken, lambda p: "") == broken[:2], "LLM ล่มต้องได้ตัวแรกๆ ไม่ใช่ลิสต์ว่าง"

    # ตอบคำเดิมซ้ำเพื่อให้ครบโควตา ต้องเหลือตัวเดียว (เจอจริงจาก LLM)
    dup = ["ไหว้เจ้า", "ซ่อมหลุมศพ", "ถอดชุดไว้ทุกข์", "ย้ายโลงศพ"]
    assert pick_recommended(dup, lambda p: "ไหว้เจ้า | ไหว้เจ้า | ไหว้เจ้า") == \
        ["ไหว้เจ้า", "ซ่อมหลุมศพ"], "ตอบซ้ำต้องเหลือตัวเดียว แล้วเติมให้ครบ 2 จากรายการ"

    # คำปิดท้ายต้องไม่ถูกส่งให้ LLM และไม่โผล่ในผลลัพธ์
    funeral = ["ไหว้เจ้า", "ซ่อมหลุมศพ", "ถอดชุดไว้ทุกข์", "แต่งชุดไว้ทุกข์",
               "เปิดหลุม", "ย้ายโลงศพ", "กิจอื่นไม่ควรทำ"]
    seen = []
    got = pick_recommended(funeral, lambda p: (seen.append(p), "ไหว้เจ้า | ซ่อมหลุมศพ")[1])
    assert "กิจอื่นไม่ควรทำ" not in seen[0], "คำปิดท้ายต้องถูกตัดก่อนส่งให้ LLM"
    assert got == ["ไหว้เจ้า", "ซ่อมหลุมศพ"]

    # 宜 มีแต่คำปิดท้าย = ไม่มีกิจกรรมจริงเลย ต้องได้ [] ไม่ใช่ยัดคำปฏิเสธไปแสดง
    assert pick_recommended(["กิจอื่นไม่ควรทำ", "ไม่มี"]) == []
    os.remove(translate_cache.CACHE_PATH)
